	...
	public void update() throws InvalidCommandException {
		...
		try {
			...
			if (!tokenMaker.check().equalsIgnoreCase("SET")) {
				...
				//--find meta data file to confirm correct columns
				File dbDirectory = new File(currentDB.name);
				...
				if(files != null) {//loop through files in db directory
					for(File file : files) {
						...
						//System.out.println(tableName.toUpperCase()+" test");
						if(file.getName().equalsIgnoreCase(tableName.toUpperCase()+".meta")) {
							tableFile = file;	
						}
					}
					if(tableFile == null) {
				...
				//go through file
				Scanner scn = new Scanner(tableFile);
				int attributeCount = Integer.parseInt(scn.nextLine().trim());//count
				scn.next();//burn second line
				ArrayList<String> colNames = new ArrayList<String>();
				while(scn.hasNextLine()) {//get names from the rest of file
				...
				if(attributeCount<setColumnsArray.length) {
					throw new InvalidCommandException("Wrong number of attributes, "+attributeCount+" expected");
				}
				
				Boolean incorrectName=false;
				
				for(int i = 0; i < setColumnsArray.length; i++) {
					Boolean nameFound = false;
					for(int j = 0; j < colNames.size(); j++) {
					//System.out.println("colNames: "+colNames.get(j) +" setNames: "+setColumnsArray[i]);
					   if(colNames.get(j).equalsIgnoreCase(setColumnsArray[i])){
						  nameFound=true;
					  }
					}
					if(!nameFound) {
				...
				if(incorrectName) {
						...
						throw new InvalidCommandException("Error: Wrong column names");
				}
				//------
				...
				if (!tokenMaker.match("FROM")) {
					...
					if(files != null) {//loop through files in db directory
						for(File file : files) {
							...
							if(file.getName().equalsIgnoreCase(tableNames[i].toUpperCase()+".meta")) {
								tableFiles.add(file);	
							}
							}
						}
						if(tableFiles.size() != tableNames.length) {
							//System.out.println("files: "+tableFiles.size()+" names: "+tableNames.length);
							throw new InvalidCommandException("Could not find table");
						}
					...
					Scanner scn = new Scanner(tableFiles.get(i));
					int attributeCount = Integer.parseInt(scn.nextLine().trim());//count
					scn.next();//burn second line
					...
					for(int i = 0; i < columnsArray.length; i++) {
						...
						for(int j = 0; j < colNames.size(); j++) {
						   ...
						   if(colNames.get(j).equalsIgnoreCase(columnsArray[i])){
							  nameFound=true;
						  }
						}
						if(!nameFound) {
					...
					if(incorrectName && !columnsArray[0].equals("*")) {
