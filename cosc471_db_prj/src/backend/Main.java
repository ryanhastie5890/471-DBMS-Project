...
public class Main {
	...
	public static void main(String [] args) throws IOException, GrammarParser.InvalidCommandException {
		...
		while(!done) {
			...
			if(upper.contains("FILE")) {//use a file for input
				runFile();
				continue;
			}
	...
	public static void runFile() throws IOException {//runs a txt file of commands line by line
		System.out.println("What is the name of your input file");
		GrammarParser parser = new GrammarParser();
		Scanner scn = new Scanner(System.in);
		
		String fileName = scn.next();//get file
		...
		while(reader.hasNext()) {
			...
			parser.setCommand(command);
			try {
			parser.beginParse();//perfor command
			}
			catch (GrammarParser.InvalidCommandException e){
				System.out.println("Command Failed, error message: "+e.getMessage());
			}
