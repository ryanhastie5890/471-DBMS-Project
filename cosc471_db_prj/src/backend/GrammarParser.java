...
public class GrammarParser {
	...
	public void create() throws IOException, GrammarParser.InvalidCommandException {// create command
		...
		if (!tokenMaker.match("CREATE")) {// checks if it is create table
			throw new InvalidCommandException("CREATE missing or incorrect");//invalid
		} else {
			if (!(tokenMaker.peek().equalsIgnoreCase("TABLE") || tokenMaker.peek().equalsIgnoreCase("DATABASE"))) {
				throw new InvalidCommandException("TABLE or DATABASE not found or incorrect");//invalid
			} else {
				...
				if (token.equalsIgnoreCase("DATABASE")) {//database case
					...
					if (!tokenMaker.match("(")) {// check that table name wasnt skipped
						...
						if (!tokenMaker.match("(")) {
							throw new InvalidCommandException("( missing or incorrect");//invalid
	...
	// inserts
		public void insert() throws InvalidCommandException, FileNotFoundException {// prototype insert command read
			...
			if (!tokenMaker.check().equalsIgnoreCase("INSERT")) {// checks if it is create table
				throw new InvalidCommandException("No INSERT found");
			} else {
				String name = tokenMaker.check();

				if (!tokenMaker.check().equalsIgnoreCase("VALUES")) {// check that table name wasnt skipped and values is ppresent

					throw new InvalidCommandException("Missing or incorrect Values");
	...
	public void describe() throws IOException, GrammarParser.InvalidCommandException {
		if (!tokenMaker.check().equalsIgnoreCase("DESCRIBE") || !tokenMaker.check().equals("(")) {
			 throw new InvalidCommandException("Missing or incorrect DESCRIBE");
		} else {
	...
	public void let() throws InvalidCommandException {
		if (!tokenMaker.check().equalsIgnoreCase("LET")) {
			throw new InvalidCommandException("Missing or incorrect LET");
		} else {
			String newTableName = tokenMaker.check();

			if (!tokenMaker.check().equalsIgnoreCase("KEY")) {
				throw new InvalidCommandException("Missing or incorrect KEY");
			} else {
	...
	public void delete() {
		try {
			...
			if(isWhere == false) {
			this.currentDB.tableNames.remove(tableName);
			}
		} catch (Exception e) {
			System.out.println(e.getMessage());
		}
