 ...
 *   TEXT          : 100 bytes
 *
 * A 1-byte "alive" flag precedes each record:
 *   0x01 = live
 *   0x00 = deleted
 */
public class StorageManager {
    ...
    // ---------------------------------------------------------------
    // DROP TABLE — removes all table files
    // ---------------------------------------------------------------
    public void dropTable(String tableName) {
        requireDb();
        String base = basePath(tableName);

        File meta = new File(base + ".meta");
        File dat  = new File(base + ".dat");
        File idx  = new File(base + ".idx");
    ...
    private void writeMeta(String metaPath, ParsedQuery q) throws IOException {
        try (PrintWriter pw = new PrintWriter(new FileWriter(metaPath))) {
            pw.println(q.columnNames.size());
            pw.println(q.primaryKey != null ? q.primaryKey.toUpperCase() : "NONE");

            for (int i = 0; i < q.columnNames.size(); i++) {
                pw.println(q.columnNames.get(i).toUpperCase() + " "
                        + q.columnTypes.get(i).toUpperCase());
            }
        }
    }
    ...
    // ---------------------------------------------------------------
    // INSERT — appends fixed-width record to .dat, updates BST if PK
    // ---------------------------------------------------------------
    public void insertRecord(String tableName, List<String> values) throws IOException, ClassNotFoundException {
        ...
        if (values.size() != meta.columnNames.size()) {
            throw new IllegalArgumentException(
                "Value count mismatch: expected " + meta.columnNames.size() + ", got " + values.size()
            );
        }

        if (meta.primaryKey != null) {
            int pkIdx = meta.getColumnIndex(meta.primaryKey);
            String pkVal = values.get(pkIdx) != null ? stripQuotes(values.get(pkIdx).trim()) : "";
        ...
        try (RandomAccessFile raf = new RandomAccessFile(datPath, "rw")) {
            ...
            for (int i = 0; i < meta.columnNames.size(); i++) {
                String raw = values.get(i) == null ? "" : values.get(i).trim();
                writeField(raf, meta.columnTypes.get(i), raw);
            }
        }

        if (meta.primaryKey != null) {
            int pkIdx = meta.getColumnIndex(meta.primaryKey);
            String pkVal = stripQuotes(values.get(pkIdx).trim());
     ...
     * Returns null if the record is deleted.
     */
    ...
    public void updateRecordAt(String tableName, long offset, List<String> newValues, TableMeta meta) throws IOException {
        try (RandomAccessFile raf = new RandomAccessFile(basePath(tableName) + ".dat", "rw")) {
            ...
            for (int i = 0; i < meta.columnNames.size(); i++) {
                String raw = newValues.get(i) == null ? "" : newValues.get(i).trim();
                writeField(raf, meta.columnTypes.get(i), raw);
            }
        }
    }
    ...
    public boolean tableExists(String tableName) {
        if (dbDirectory == null) return false;
    ...
    private void writeField(RandomAccessFile raf, String type, String value) throws IOException {
        switch (type.toUpperCase()) {
            case "INT":
            case "INTEGER": {
                int v = value.isEmpty() ? 0 : (int) Double.parseDouble(stripQuotes(value));
                raf.writeInt(v);
                break;
            }
            case "FLOAT": {
                double v = value.isEmpty() ? 0.0 : Double.parseDouble(stripQuotes(value));
                raf.writeDouble(v);
                break;
            }
            case "TEXT":
    ...
    private String readField(RandomAccessFile raf, String type) throws IOException {
        switch (type.toUpperCase()) {
            ...
            case "FLOAT":
                return String.valueOf(raf.readDouble());
